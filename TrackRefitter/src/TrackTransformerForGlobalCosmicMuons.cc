...
vector<Trajectory> TrackTransformerForGlobalCosmicMuons::transform(const reco::Track& tr) const {
  ...
  //AR: changed below following guidelines at: https://twiki.cern.ch/twiki/bin/viewauth/CMS/MuonAlignmentReferenceTarget  
  ...
  vector<Trajectory> trajectories2 = fitter(up)->fit(trajectoryFW2.seed(),trajectoryFW2.recHits(),trajectoryFW2.geometricalInnermostState());
  if(trajectories2.empty()){
    LogTrace(metname)<<"No Track refitted!"<<endl;
    return vector<Trajectory>();
  }
  Trajectory trajectoryBW2 = trajectories2.front();
  vector<Trajectory> trajectoriesSM2 = smoother(up)->trajectories(trajectoryBW2);
  ...
  vector<Trajectory> trajectories3 = fitter(up)->fit(trajectoryFW3.seed(),trajectoryFW3.recHits(),trajectoryFW3.geometricalInnermostState());
  if(trajectories3.empty()){
    LogTrace(metname)<<"No Track refitted!"<<endl;
    return vector<Trajectory>();
  }
  Trajectory trajectoryBW3 = trajectories3.front();
  vector<Trajectory> trajectoriesSM3 = smoother(up)->trajectories(trajectoryBW3);
